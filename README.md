...
# 수업 진행 
- (수업 전에) 학생들에게 단어 리스트를 제시할 것(영-한으로 고치기)
- 위밍업 워들 : 영영풀이와 규칙 동시에 제공
  
- 롤플레이 : TTS/선택적 텍스트 제공/AB나누기/ 문법을 치지 않고 버튼식으로> 문법 설명(줄글)과 예문 제공
- 문법 리뷰 영상(1~2분 길이)
- 본문: TTS+TEXT+IMAGE
- TTS로 한 문장씩 들을 수 있도록 > 새도잉
- dictation(쉬운거)

- 퀴즈(영영풀이, TF, 문법배열)

학생 이모티콘
- 본문; 내용; dictation(난이도 조절해서 할 수 있도록) > 개별학습
- 단어 영영풀이 매칭 > 개별학습
- 문법 리뷰 영상(1~2분 길이)+확인문제(문법 튜토리얼 비디오를 삽입 (받아쓰기처럼 빈칸)//수동태, 과거형/분사형, 관계사)

- 새 아이디어?
  - (단어장 미리 준 경우)랜덤 영단어>그림>업로드>공유>단어카드
  - 단어 제시 > 그림> 패들릿> 그림동화 만들기 (본문수업 후 추가활동)
  - TTS+본문+이미지 동시에 제공
  - 뒷이야기 이어 쓰기
  
  - 내용 상상해서 쓰기> 생각해야하는 질문 정해서 조별로 하나씩 부여> 발표 (발표 또는 패들릿)
